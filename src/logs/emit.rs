...
pub(crate) fn agent_info(message: impl AsRef<str>) {
    eprintln!("{}", agent_line(LogLevel::Info, message.as_ref()));
}

pub(crate) fn agent_warn(message: impl AsRef<str>) {
    eprintln!("{}", agent_line(LogLevel::Warn, message.as_ref()));
}

pub(crate) fn agent_error(message: impl AsRef<str>) {
    eprintln!("{}", agent_line(LogLevel::Error, message.as_ref()));
}

pub(crate) fn agent_debug(message: impl AsRef<str>) {
    if verbose_enabled() {
        eprintln!("{}", agent_line(LogLevel::Debug, message.as_ref()));
    }
}
...
pub(crate) fn job_phase_info(job_id: &str, phase: &str, message: impl AsRef<str>) {
    eprintln!(
        "{}",
        job_phase_line(job_id, phase, LogLevel::Info, message.as_ref())
    );
}
...
pub(crate) fn job_phase_warn(job_id: &str, phase: &str, message: impl AsRef<str>) {
    eprintln!(
        "{}",
        job_phase_line(job_id, phase, LogLevel::Warn, message.as_ref())
    );
}
...
pub(crate) fn job_phase_error(job_id: &str, phase: &str, message: impl AsRef<str>) {
    eprintln!(
        "{}",
        job_phase_line(job_id, phase, LogLevel::Error, message.as_ref())
    );
}
...
pub(crate) fn job_stream_line(
    ...
    eprintln!(
        "{}",
        format_job_stream_line(&ctx.job_id, phase, stream.as_str(), &line)
    );
