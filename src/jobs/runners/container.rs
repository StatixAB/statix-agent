...
impl LxcContainer {
    ...
    fn configure_guest_dns(&self) -> Result<()> {
        ...
        let resolv_conf_path = self.rootfs_path().join("etc").join("resolv.conf");
        if fs::symlink_metadata(&resolv_conf_path).is_ok() {
            fs::remove_file(&resolv_conf_path)
                .with_context(|| format!("failed to remove {}", resolv_conf_path.display()))?;
        }

        let mut contents = String::from("# Generated by statix-agent for job container DNS\n");
        ...
        fs::write(&resolv_conf_path, contents)
            .with_context(|| format!("failed to write {}", resolv_conf_path.display()))?;
        eprintln!(
